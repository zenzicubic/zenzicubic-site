...
import { MoebiusTransformation, Circle } from './steiner_utils';
import { Complex, complex, cis, sqr, Re } from '../../math';
...
function SteinerDemo() {
    ...
    */
    ...
    const transformCircle = useCallback((circ) => {
        ...
        let den = tran.d.div(tran.c).add(circ.cen).conj();
        let pt = circ.cen.sub(den.inv().mulRe(sqr(circ.rad)));

        let cen = tran.transform(pt);
        let rad = cen.dist(tran.transform(circ.cen.add(Re(circ.rad))));
    ...
    const drawCircles = useCallback(() => {
        ...
        for (let i = 0; i < N; i ++) {
            ...
            drawCircle(transformCircle(circ));
        }
